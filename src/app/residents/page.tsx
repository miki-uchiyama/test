...
export default function ResidentsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.office_id) { showMessage('error', '所属事業所を選択してください'); return }
    setSaving(true)

    if (editingId) {
      const { error } = await supabase
        .from('residents')
        .update({ name: form.name, office_id: form.office_id, display_order: form.display_order, is_active: form.is_active })
        .eq('id', editingId)
      ...
      const { error } = await supabase
        .from('residents')
        .insert({ name: form.name, office_id: form.office_id, display_order: form.display_order, is_active: form.is_active, created_by: currentUserEmail })
  ...
  const handleEdit = (r: ResidentRow) => {
    setEditingId(r.id)
    setForm({ name: r.name, office_id: r.office_id, display_order: r.display_order, is_active: r.is_active })
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  const handleToggleActive = async (r: ResidentRow) => {
            ...
            <div>
              ...
              {offices.length === 0 ? (
                <p className="text-sm text-red-500 mt-2">
                  事業所が登録されていません。先に Supabase の offices テーブルに事業所を追加してください。
                </p>
              ) : (
                <select
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {displayed.map((r) => (
                    <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${!r.is_active ? 'opacity-50' : ''}`}>
                      <td className="table-cell font-medium text-gray-800">{r.name}</td>
                      <td className="table-cell text-gray-600">{r.offices?.name ?? '−'}</td>
                      <td className="table-cell text-gray-500">{r.display_order}</td>
